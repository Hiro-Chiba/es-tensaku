...
## 11. エンタメ機能拡張: 大富豪NPCプレイヤー
- **目的**: ワンプレイヤーでも楽しめるよう、大富豪対戦用のNPCを実装し、規定人数に満たない場合でもゲームを成立させる。
- **要件**:
  - プレイヤー人数が不足した際には日本人名のNPCで自動補充すること。
  - NPCは高度な戦略ロジックを備え、手札解析・場の状況を踏まえた強力な意思決定を行う。
  - ロジックは単純なランダムではなく、複数の評価軸（手札消化効率、革命判定、終盤対応など）を組み合わせる。
- **成果物**: NPCロジックモジュール、名前生成ユーティリティ、ユニットテスト。
